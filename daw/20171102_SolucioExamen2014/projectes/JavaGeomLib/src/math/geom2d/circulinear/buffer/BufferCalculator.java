...
public class BufferCalculator {
	...
	public CirculinearBoundary2D createParallelBoundary(
		...
		return CirculinearContourArray2D.createCirculinearContourArray2D(parallelContours);
	}
	 ...
	 * <li> createPolyCirculinearCurve2D a new domain with the final set of contours
 </ol>
	 */
	public CirculinearDomain2D computeBuffer(
		...
		// All the rings are created, we can now createPolyCirculinearCurve2D a new domain with the
		// set of rings
		return new GenericCirculinearDomain2D(
				CirculinearContourArray2D.createCirculinearContourArray2D(contours2));
	}
	...
	computeBufferSimpleCurve(CirculinearContinuousCurve2D curve, double d) {
		...
		if (curve.isClosed()) {
			...
			// createPolyCirculinearCurve2D a new contour from the two parallels and 2 caps
	...
	createSingleContourFromTwoParallels(
		...
		// createPolyCirculinearCurve2D new ring using two open curves and two circle arcs
		if (curve1 != null && curve2 != null){
			...
			if (b0 && b1) {
					...
					// createPolyCirculinearCurve2D the last ring
				...
				// createPolyCirculinearCurve2D the last contour
				contours.add(new GenericCirculinearRing2D(elements));
